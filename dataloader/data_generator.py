...
class DataGenerator(Dataset):
    def __init__(self, fg_path=None, bg_path=None, merged_path=None, alpha_path=None, trimap_path=None, phase="train"):
        self.phase = phase
        self.crop_size = 512
        

        
        #[alpha_path+x for x in sorted(os.listdir(alpha_path))]
        # self.merged = [merged_path+x for x in sorted(os.listdir(merged_path))]
        ...
        if self.phase == "train":
            ...
            self.bg = [bg_path+x for x in sorted(os.listdir(bg_path))]
            self.fg = [fg_path+x for x in sorted(os.listdir(fg_path))]
            self.alpha = [alpha_path+x for x in sorted(os.listdir(alpha_path))]
            ...
            self.bg = []
            self.merged = [merged_path+x for x in sorted(os.listdir(merged_path))]
            self.trimap = [trimap_path+x for x in sorted(os.listdir(trimap_path))]

        train_trans = [
            RandomAffine(degrees=30, scale=[0.8, 1.25], shear=10, flip=0.5),
            GenMask(),
            CutMask(perturb_prob=0.25),
    ...
    def __getitem__(self, idx):
        if self.phase == "train":
            ...
            # print (self.fg[idx % self.fg_num], self.bg[idx])
            
            # fg = cv2.imread(self.fg[idx % self.fg_num])
            # alpha = cv2.imread(self.alpha[idx % self.fg_num], 0).astype(np.float32)/255

            # bg = np.load(self.bg[idx])
            bg = cv2.imread(self.bg[idx])
            
            bg_name = self.bg[idx].split('/')[-1]
            fg_name = bg_name.split('_'+bg_name.split("_")[-1])[0]+'.jpg'

            
#             print (self.bg[idx])
#             print (self.fg_path + fg_name)
#             print (self.alpha_path + fg_name)
            
            
            fg = cv2.imread(self.fg_path + fg_name)
            alpha = cv2.imread(self.alpha_path + fg_name, 0).astype(np.float32)/255
            ...
            image_name = bg_name#os.path.split(self.fg[idx % self.fg_num])[-1]
