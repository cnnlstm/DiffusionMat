...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        # print ('saasdsacadscas',trimap.max(),trimap.min())
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        image, alpha, trimap, mask, name = sample['image'],  sample['alpha'], sample['trimap'], sample['mask'], sample['image_name']
        #bg = sample['bg']
        h, w = trimap.shape
        ...
        sample.update({'image': image_crop, 'alpha': alpha_crop, 'trimap': trimap_crop, 'mask': mask_crop})
        return sample
...
class DataGenerator(Dataset):
    def __init__(self, merged_path, alpha_path, trimap_path=None, phase="train"):
        self.phase = phase
        self.crop_size = 512
        ...
        if self.phase == "train":
            ...
            self.trimap = [trimap_path+x for x in trimap_path]
        ...
        self.merged_num = len(self.merged)

    def __getitem__(self, idx):
        if self.phase == "train":
            # print (self.merged[idx % self.merged_num])
            # print (self.alpha[idx % self.merged_num])
            
            image = cv2.imread(self.merged[idx % self.merged_num])
            alpha = cv2.imread(self.alpha[idx % self.merged_num], 0).astype(np.float32)/255
            #bg = cv2.imread(self.bg[idx], 1)
            #fg, alpha = self._composite_fg(fg, alpha, idx)

            image_name = os.path.split(self.merged[idx % self.merged_num])[-1]
